...
plt.figure(figsize=(40, 20)) #输出第一张图
...
plt.show()
plt.figure(figsize=(40,20)) #输出第二张图
for column_index, column in enumerate(df.columns[2:3]):
    ...
    sb.violinplot(x='species', y=column, data=df)
plt.show()
plt.figure(figsize=(40,20)) #输出第三张图
for column_index, column in enumerate(df.columns[3:4]):
    ...
    sb.violinplot(x='species', y=column, data=df)
plt.show()
plt.figure(figsize=(40,20)) #输出第四张图
...
plt.figure(figsize=(40,20)) #输出第五张图
...
def EnglishtoChinese(english):    #自定义函数实现英文转为中文。
    if english=="Zinfandel":
        return "仙粉黛"
    elif english=="Sauvignon":
        return "赤霞珠"
    elif english=="Syrah":
        return "西拉"
print("X_test中的全部数据分类的结果为:")#汉字提醒
model.predict(X_test[:])
count=0
for english in model.predict(X_test[:]):
    english1=EnglishtoChinese(english) # 输出测试的结果的具体中文，即输出模型预测的结果
    count=count+1
    print(english1,end=" ") #调整排版
    if count%5==0:
        print("")
print(" ")
print("----------------------")
...
data4=EnglishtoChinese(data1)
data5=EnglishtoChinese(data2)
data6=EnglishtoChinese(data3)
